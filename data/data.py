...
class DetectFeatLmdb(object):
    def __init__(self, img_dir, conf_th=0.2, max_bb=100, min_bb=10, num_bb=100,
                 compress=True):
        ...
        # test
        #env = lmdb.open(f'{img_dir}{db_name}/data.mdb')
        #env = lmdb.open("/img/reverie_train/test")
    ...
    def __getitem__(self, file_name):
        # test start
        """
        "reverie_VLzqgDo317F_f0244e1e73e34c6997e25f07e9b42328_10.npz"
        file_name_test = "reverie_p5wJjkQkbXX_6e2076826afb43a0943697f496beb998_14.npz"
        dump_test = self.txn.get(file_name_test.encode('utf-8'))
        img_dump = msgpack.loads(dump_test, raw=False)
        file_name_test = "reverie_p5wJjkQkbXX_6e2076826afb43a0943697f496beb998_15.npz"
        dump_test = self.txn.get(file_name_test.encode('utf-8'))
        img_dump = msgpack.loads(dump_test, raw=False)
        """
        # test end
        ...
        if self.compress:
            ...
            img_dump = msgpack.loads(dump, raw=False)
        if img_dump['features'].shape[0] > nbb or img_dump['norm_bb'].shape[0] > nbb:
            print(img_dump['features'].shape[0])
        img_feat = torch.tensor(img_dump['features']).float()
        img_bb = torch.tensor(img_dump['norm_bb']).float()
        object_id = torch.tensor(img_dump['box_id']).float()

        return img_feat, img_bb, object_id
...
class DetectFeatTxtTokDataset(Dataset):
    ...
    def _get_img_feat(self, fname):
        img_feat, bb, object_id = self.img_db[fname]
        img_bb = torch.cat([bb, bb[:, 4:5]*bb[:, 5:]], dim=-1)
        num_bb = img_feat.size(0)
        return img_feat, img_bb, num_bb, object_id
